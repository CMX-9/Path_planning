def do_a_star(grid, start, end, display_message):
    ...
    [Improvement Process Record]
        Evaluation Criteria: Primarily tested algorithm runtime speed, memory and CPU usage performance, compared expanded nodes, path nodes, and open list node counts.
        Improvement Process：
            - Initial Implementation Phase：During testing of the first version of the code, it was discovered that when multiple nodes shared the same f-value, 
            the algorithm did not prioritize expanding nodes with smaller h-values.Initially, the idea was to prioritize h-values when f-values were equal. 
            However, empirical testing showed this significantly increased sorting computational load, leading to noticeable runtime slowdowns. This approach was subsequently abandoned.
            - Data Structure Evolution：
            Version 1 (Dictionary-based)，advantages are Simple and intuitive implementation.Disadvantages: Significant slowdowns and potential crashes as grid sizes increased.
            Version 2 (2D Array Optimization),Key Improvements:Reduced heuristic recomputation by 95% via precomputing h_cache.Maintained open_list order via insertion sorting, 
            achieving substantial speed improvements in complex scenarios.Limitations: Memory inefficiencies emerged for very large grids.
            Version 3 (1D Memory Compression),Final Optimization: Linearized coordinates for memory efficiency in massive grids.
        Future Optimization Directions: 
        In addition to the impact of grid size, obstacle complexity also significantly affects algorithm runtime and resource consumption. 
        Future plans include integrating deep learning and stack-based approaches for further optimization, as well as methods like bidirectional A* for bidirectional search in symmetric environments.
    """
    ...
    elif 900 < grid_size <= 250000:                                 # 500x500=250,000 cells threshold
        """
    ...
    display_message(f"[SUCCESS] Path found with {len(path)} nodes")
    return path