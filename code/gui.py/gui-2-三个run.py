...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        # 添加新按钮变量
        self.run_button_algorithm3 = QPushButton("Run Algorithm 3")
    ...
    def draw_control_panel(self, parentLayout):
        ...
        # 创建算法3按钮
        self.run_button_algorithm3 = QPushButton("Run Algorithm 3")
        self.run_button_algorithm3.setStyleSheet("QPushButton { background-color: white }")
        self.run_button_algorithm3.setMaximumHeight(self.width_input.height())
        self.run_button_algorithm3.clicked.connect(self.on_click_run_algorithm3)
        ...
        control_panel_layout.addWidget(self.run_button_algorithm3)  # 添加新按钮
        control_panel_layout.addStretch(1)
    ...
    def reset(self):
        ...
        self.canvas.path_algorithm3 = None  # 新增
        
        self.start_set = False
        self.canvas.start = None
        self.canvas.end = None
        self.algorithm_generator = None
        self.canvas.update()  # 强制刷新画布
    ...
    def on_click_run_algorithm_b(self):
        ...
        try:
            ...
            self.display_message(f"错误: {str(e)}", "ERROR")
    # 新增算法3执行方法
    def on_click_run_algorithm3(self):
        self.canvas.path_algorithm3 = None
        self.canvas.update()
        ...
        try:
            importlib.reload(algorithm3)  # 假设算法3模块名为algorithm3
            path = algorithm3.do_a_star(
                self.create_grid(),
                self.canvas.start,
                self.canvas.end,
                self.display_message,
            ...
            self.canvas.path_algorithm3 = path
            self.canvas.update()
...
class CanvasWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.path_algorithm3 = None  # 新增路径属性
        self.start = None
        self.end = None
        self.cell_width = 0
        self.cell_height = 0
    ...
    def paintEvent(self, event):
        ...
        # 绘制算法3路径（使用黄色）
        if self.path_algorithm3:
            painter.setBrush(Qt.yellow)
            for cell in self.path_algorithm3:
                painter.drawRect(*self.cell_to_coords(cell))
